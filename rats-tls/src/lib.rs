...
impl RatsTls {
    ...
    // pub fn negotiate_tcpstream(self, stream: TcpStream) -> Result<RatsTlsSession, rats_tls_err_t> {
    //     let err = unsafe { rats_tls_negotiate(self.as_ptr(), stream.as_raw_fd()) };
    //     if err == RATS_TLS_ERR_NONE {
    //         Ok(RatsTlsSession::new(self, stream))
    //     } else {
    //         Err(err)
    //     }
    // }
...
struct RatsTlsSession {
    ratstls: Arc<(RatsTls, TcpStream)>, // hold TcpStream as owner of fd
    task_receive: Option<JoinHandle<Result<(Vec<u8>, usize), rats_tls_err_t>>>,
}

impl RatsTlsSession {
    fn new(ratstls: RatsTls, stream: TcpStream) -> Self {
        RatsTlsSession {
            ratstls: Arc::new((ratstls, stream)),
            task_receive: None,
        }
    }
}

// impl AsyncRead for RatsTlsSession {
//     fn poll_read(
//         mut self: std::pin::Pin<&mut Self>,
//         cx: &mut std::task::Context<'_>,
//         buf: &mut tokio::io::ReadBuf<'_>,
//     ) -> std::task::Poll<std::io::Result<()>> {
//         if self.task_receive.is_none() {
//             let clone = self.ratstls.clone();
//             let task = tokio::task::spawn_blocking(move || {
//                 let mut buf = vec![0u8; 1024]; // TODO: buffer len
//                 clone.0.receive(&mut buf).map(|len| (buf, len))
//             });
//             self.task_receive = Some(task)
//         }

//         let handle = self.task_receive.as_mut().unwrap();
...
//                     Poll::Ready(Ok(()))
//                 }
//                 Ok(Err(err)) => Poll::Ready(Err(std::io::Error::new(
//                     ErrorKind::Other,
//                     format!("Failed in RatsTLS::receive(): error code {}", err),
//                 ))),
//                 Err(err) => Poll::Ready(Err(std::io::Error::new(
//                     ErrorKind::Other,
//                     format!("Failed when calling RatsTLS::receive(): {}", err),
//                 ))),
//             },
